...
// Load Tx from file
Tx.fromFile = async hash =>
  Tx.fromBuffer(await fs.readFile(`../data/${config.network}/tx/${hash}`))


// Save Tx to file
Tx.save = async tx => {
  return await fs.outputFile(`../data/${config.network}/tx/_${tx.getId()}`, tx.toBuffer())
}
...
const parseOutput = (el, { network = 'mainnet' } = {}) => {
  ...
  try {
    ...
    //   case 'pubkey':
    //     // var pubKeyBuffer=new Buffer(output.scriptPubKey.asm.split(' ')[0],'hex')
    //     // bitcoinJS.ECPair.fromPublicKeyBuffer(pubKeyBuffer).getAddress()
    //     break
    //   case 'pubkeyhash':
    //   case 'scripthash':
    //     output.address = Address.fromOutputScript(el.script, network)
    //     break
    // }
